    ...
    def get_graph(self, conn: GraphDBConnection,
        ...
        for node_config in node_export_config:
            ...
            for edge_config in edge_export_config:
                ...
                query = f"SELECT ?srcID ?dstID ?p WHERE {{BIND(<{node1_name}> AS ?src) BIND(<{node2_name}> AS ?dst) ?src ?p ?dst. ?src <id> ?srcID. ?dst <id> ?dstID . ?p a <{edge_name}>}}"
...
## get_graph
    ...
    # edge1 = EdgeExportConfig('Cite', ('paper/163','paper/402'),[],[])
